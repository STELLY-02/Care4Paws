    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(
                'http://localhost:8085/api/pets',  // Changed from /pets to /api/pets
                formDataToSend,
                {
                    headers: {
    ...
    const handleDelete = async (petId) => {
        if (window.confirm('Are you sure you want to delete this pet? This action cannot be undone.')) {
            try {
                ...
                await axios.delete(`http://localhost:8085/api/pets/${petId}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    const handleUpdate = async (updatedPetData) => {
        try {
            ...
            const response = await axios.put(
                `http://localhost:8085/api/pets/${editingPet._id}`,
                updatedPetData,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                                                ...
                                                return (
                                                    <div key={pet._id} className="pet-card">
                                                        <div className="pet-image">
                                                            <img 
                                                                src={`http://localhost:8085${pet.photo}`}
                                                                alt={pet.name}
                                                                style={{ 
                                                                    width: '100%', 
                                                                    height: '200px', 
